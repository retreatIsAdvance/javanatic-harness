...
- 实现变化按 [设计同步触发规则](../plan/README.md#设计同步触发规则) 更新对应设计；既有 pre-release 自由重排措辞不得覆盖 [API 稳定面](12-api-stability.md)，其指令文档冲突在后续获授权的同步中处理。
- **下一迭代：it17「CLI 任务结果闭环与真实任务基线」**；具体契约、锚点和审查停点留待该迭代四确认，本节不替代开工手续。
