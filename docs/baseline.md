...
任务 04–08 会让 Agent 跑 maven。快照首次构建超过 shell 工具的超时（组合期上限 **60s**，不可由模型调大），建议先在各 case 目录预热一次（约 1–2 分钟/目录）：
...
- shell 命令 60s 硬超时（见上）；`mvn` 全量构建（`mvn -B package`）必然超时，任务文本里没有它。
