...
- **内容**：
  ...
  4. **双语 README**：`README.md` → 英文（国际主读者）；`README.zh-CN.md` 中文全本（现内容刷新，计数订正：46 个 reactor 项目 / 365 用例）；置顶互链；设计文档维持中文（语言政策不动，README 说明）。
...
1. **命名空间拼写**：`io.github.retreatIsAdvance` 与 GitHub 用户名精确一致（GitHub 登录后系统自动创建 `io.github.<username>`；验证凭 GitHub 身份，不需临时仓）。若 Portal 回显拼写不同（如全小写），一次 sed 校正，发布前零成本。
...
4. **版本翻转时机**：`0.1.0-SNAPSHOT → 0.1.0`、tag、bump `0.2.0-SNAPSHOT` 都在发布执行时做（S3 文档化命令序列）；平时保留 SNAPSHOT。
...
| 锚点（文件:符号） | 预期改动 | 完成 |
|---|---|---|
| 46 个 `pom.xml`（253 处 `<groupId>`） | → `io.github.retreatIsAdvance`（exact 元素匹配，JPMS 名不动） | ☐ |
| `pom.xml`（根）：project 元数据 | url / scm / licenses / developers | ☐ |
| `pom.xml`（根）：`<profiles>` + pluginManagement | `release` profile（四插件 + 版本） | ☐ |
| `examples/pom.xml`、`dist/pom.xml` | `maven.deploy.skip=true` | ☐ |
...
| 停点 | 覆盖锚点/类 | 状态 |
|---|---|---|
| S1 发布工装 + 坐标改名 | 46 POM + 根 POM 元数据/release profile + deploy.skip + 文档 4 处 | ☐ |
...
- [ ] S1 改名后全量绿：`mvn -B -q validate` + 全量 `mvn -B -q install`（改名触全仓；顺带回填本地仓库新坐标）
- [ ] S1 release 剖面工件完整：`mvn -B -P release -Dgpg.skip=true package` 全量绿；抽查模块 target 含 jar + sources.jar + javadoc.jar；effective-pom 检查元数据（url/licenses/developers/scm）逐模块生效
- [ ] S1 签名链路（隔离测试密钥，若策略不拦）：一次性 GNUPGHOME 生成测试密钥 → `-P release` 真签名 → 抽查 .asc 与 `gpg --verify` 通过
