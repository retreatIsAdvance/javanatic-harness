# 迭代 17 — CLI 任务结果闭环与真实任务基线（状态：进行中——S1 已实现取证、停点待放行；S2 未开始）
...
- **内容**：
  ...
  5. **真实任务基线（用户执行）**：新增 `docs/baseline.md`——固定 10 个小型任务（定位 ×3、修改+验证 ×3、测试运行 ×1、重构 ×1、文档 ×2；种子 = `git archive` 钉提交的快照工作区），含记录协议（退出码/stdout 判读、耗时、output tokens 汇总命令、主观判定）。**10 次真跑由用户执行（计费决定权 / 凭证不进会话 / 任务达成的人工判断均归用户），会话只备料（任务清单、种子命令、判读与记录模板）**；S2 停点即交接点。27/30 成功率目标是 0.2.0 发布验收（由后续轮次积累），非本迭代判据。
...
| `docs/baseline.md`（新） | 任务集 + 记录协议 + 首轮记录位（真跑由用户执行，S2 交接） | 未开始（S2） |
...
| 停点 | 覆盖锚点/类 | 状态 |
|---|---|---|
| S1 CLI 结果契约（stdout 语义 + 退出码 + 失败文案；--help/12/README/AGENTS 同步；聚焦测试） | `HeadlessMain` + 12 §6 + README ×2 | packet 已出，待放行 |
| S2 **交接点**：基线任务集定稿 + 备料交付（任务清单 / 种子命令 / 判读与记录模板）；**10 次真跑由用户执行**，会话不执行真跑、不进计费与凭证 | `docs/baseline.md` + 用户真跑 | 待放行 |
...
- [ ] 首轮基线：`docs/baseline.md` 备料交付（任务集 + 协议 + 模板）→ 用户执行 10 次真跑后回收记录（本轮回收不阻塞 S1 放行）
