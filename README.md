...
| 12.6 | 硬化（**待排**，视需要插入）：JSONL fsync/撕裂尾、LlmError 分类、LocalFs realpath、`--verify` 在无同机后端平台的预警、`LoopGuard` 滞后注释、02 遗留漂移订正（Core/LLM 表缺 core-todo/plan/preset 与 openai-compat、deepseek 行描述、Jackson 归属注、§5 目录树） | — |
